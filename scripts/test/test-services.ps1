...
try {
    ...
    Write-Host "  ✓ Second account created successfully" -ForegroundColor Green
...
try {
    ...
    Write-Host "  ✓ Idempotency working - Returned existing transfer" -ForegroundColor Green
    Write-Host "    Transfer Reference: $($duplicateResponse.data.transferReference)" -ForegroundColor Cyan
    
} catch {
    if ($_.Exception.Response.StatusCode -eq 409) {
        Write-Host "  ✓ Idempotency working - Duplicate prevented (409 Conflict)" -ForegroundColor Green
    } else {
...
Write-Host "  ✓ Account creation works" -ForegroundColor Green
Write-Host "  ✓ Transfer execution works (SAGA pattern)" -ForegroundColor Green
Write-Host "  ✓ Idempotency protection works" -ForegroundColor Green
Write-Host ""
